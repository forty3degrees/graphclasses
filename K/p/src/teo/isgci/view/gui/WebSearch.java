...
import teo.isgci.core.util.BinaryPredicate;
import teo.isgci.core.util.LessLatex;
import teo.isgci.core.util.Utility;
import teo.isgci.data.db.*;
import teo.isgci.view.gui.ISGCIMainFrame;
import teo.isgci.view.gui.MessageDialog;
import teo.isgci.view.gui.NodeList;
...
public class WebSearch extends JTextField implements Iterator {
    ...
    public boolean setListData(ISGCIMainFrame parent, NodeList list) {
        String text = getText();
        if (text.length() == 0) {
            list.setListData(App.DataProvider.getGraphClasses());
        } else {
            boolean error = false;
            try {
                search(text, true);
    ...
    public Object next() {
        if (count < ergebnis.size()) {
            return App.DataProvider.getClass(ergebnis.get(count++));
        } else
