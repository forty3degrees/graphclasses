...
protected ResizableIcon getIcon(String ressourceString) {
	...
	protected JRibbonBand getLayoutBand() {
		...
		JCommandButton layGraph = new JCommandButton("Layout", getIcon("images/Layout.png"));
		...
		JCommandButton optGraph = new JCommandButton("Preferences", getIcon("images/Preferences.png"));
		optGraph.setActionKeyTip("P");
		optGraph.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				final ActionListener layoutListener = new ActionListener() {
		          ...
		          OptionSupport.showDialog(m_parent.groupLayoutOptions, layoutListener, false, m_parent.view.getFrame());
		          configureGroupLayout();
			}
		});
		layoutBand.addCommandButton(optGraph,
				RibbonElementPriority.TOP);
	...
	void configureGroupLayout() {
        Object gsi = m_parent.groupLayoutOptions.get("Group Layering Strategy");
        if ("Recursive Layering".equals(gsi)) {
        	m_parent.layouter.setRecursiveGroupLayeringEnabled(true);
        } else if ("Global Layering".equals(gsi)) {
        	m_parent.layouter.setRecursiveGroupLayeringEnabled(false);
        }

        m_parent.layouter.setGroupCompactionEnabled(m_parent.groupLayoutOptions.getBool("Enable Compact Layering"));

        Object gai = m_parent.groupLayoutOptions.get("Group Alignment");
        if ("Top".equals(gai)) {
        	m_parent.layouter.setGroupAlignmentPolicy(IncrementalHierarchicLayouter.POLICY_ALIGN_GROUPS_TOP);
        } else if ("Center".equals(gai)) {
        	m_parent.layouter.setGroupAlignmentPolicy(IncrementalHierarchicLayouter.POLICY_ALIGN_GROUPS_CENTER);
        }
        if ("Bottom".equals(gai)) {
        	m_parent.layouter.setGroupAlignmentPolicy(IncrementalHierarchicLayouter.POLICY_ALIGN_GROUPS_BOTTOM);
        }
      }
