...
public class OpenProblemDialog extends JDialog
    ...
    public OpenProblemDialog(ISGCIMainFrame parent, String problem) {
        ...
        this.parent = parent;
        this.problem = App.DataProvider.getProblem(problem);
    ...
    private void initListOpen() {
        Vector v = new Vector();
        for (GraphClass gc : App.DataProvider.getGraphClasses()) {
    ...
    private void initListsBoundary() {
        ...
        for (Inclusion e : App.DataProvider.getInclusionGraph().edgeSet()) {
            ...
            if (cfrom.likelyNotP()  &&  !cto.equals(cfrom)) {
                npc.addAll(App.DataProvider.getEquivalentClasses(e.getSuper()));
            }
            if (cto.betterOrEqual(Complexity.P)  &&
                    (cfrom.isUnknown() || cfrom.likelyNotP())) {
                p.addAll(App.DataProvider.getEquivalentClasses(e.getSub()));
            }
        }
